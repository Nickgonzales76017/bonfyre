...
typedef BfSha256 sha256_ctx;

static void sha256_init(sha256_ctx *c) { bf_sha256_init(c); }
static void sha256_update(sha256_ctx *c, const uint8_t *data, size_t len) { bf_sha256_update(c, data, len); }
static void sha256_final(sha256_ctx *c, uint8_t out[32]) { bf_sha256_final(c, out); }
...
static int init_peer_db(swarm_state_t *state, const char *override_path) {
    ...
    if (override_path && override_path[0]) {
        ...
        char dir[PATH_MAX];
        snprintf(dir, sizeof(dir), "%s", override_path);
...
static int accept_peer(swarm_state_t *state) {
    ...
    /* Peer messages (BT_REQUEST etc.) are handled without blocking the
     * accept loop. */
    fcntl(fd, F_SETFL, fcntl(fd, F_GETFL, 0) | O_NONBLOCK);

    /* Durable record of who connected: without this, `bonfyre-swarm peers`
     * always reports zero peers regardless of real connections, since
     * nothing else populates the peers table. */
    if (state->index_db) {
        char peer_hex[41];
        char addr_text[INET6_ADDRSTRLEN] = "";
        int peer_port = 0;
        hex_encode(p->peer_id, PEER_ID_LEN, peer_hex);
        if (from.ss_family == AF_INET) {
            ...
            inet_ntop(AF_INET6, &sin6->sin6_addr, addr_text, sizeof(addr_text));
            peer_port = ntohs(sin6->sin6_port);
        }
        sqlite3_stmt *stmt = NULL;
        sqlite3_prepare_v2(state->index_db,
            "INSERT INTO peers (peer_id, addr, port, info_hash, gate_token, last_seen) "
            "VALUES (?, ?, ?, ?, ?, datetime('now')) "
            "ON CONFLICT(peer_id, info_hash) DO UPDATE SET "
            "addr=excluded.addr, port=excluded.port, last_seen=excluded.last_seen;",
            -1, &stmt, NULL);
        sqlite3_bind_text(stmt, 1, peer_hex, -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 2, addr_text, -1, SQLITE_TRANSIENT);
        sqlite3_bind_int(stmt, 3, peer_port);
        sqlite3_bind_text(stmt, 4,
            state->artifact_count > 0 ? state->artifacts[0].sha256_hex : "", -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(stmt, 5, state->gate_key[0] ? state->gate_key : "", -1, SQLITE_TRANSIENT);
        sqlite3_step(stmt);
        sqlite3_finalize(stmt);
    }
...
/* ── Service one pending request from an already-connected peer ─── */

static void service_peer_requests(swarm_state_t *state) {
    for (int i = 0; i < state->peer_count; i++) {
        swarm_peer_t *peer = &state->peers[i];
        uint8_t msg[17];
        ssize_t n = recv(peer->fd, msg, sizeof(msg), MSG_DONTWAIT);
        if (n <= 0) continue;
        if (n != 17) continue;
        uint32_t msglen = ((uint32_t)msg[0] << 24) | ((uint32_t)msg[1] << 16) |
                          ((uint32_t)msg[2] << 8) | (uint32_t)msg[3];
        if (msglen != 13 || msg[4] != BT_REQUEST) continue;
        uint32_t piece_idx = ((uint32_t)msg[5] << 24) | ((uint32_t)msg[6] << 16) |
                             ((uint32_t)msg[7] << 8) | (uint32_t)msg[8];
        uint32_t offset = ((uint32_t)msg[9] << 24) | ((uint32_t)msg[10] << 16) |
                          ((uint32_t)msg[11] << 8) | (uint32_t)msg[12];
        uint32_t length = ((uint32_t)msg[13] << 24) | ((uint32_t)msg[14] << 16) |
                          ((uint32_t)msg[15] << 8) | (uint32_t)msg[16];
        if (state->artifact_count > 0 && length <= PIECE_SIZE) {
            serve_piece(state, peer, state->artifacts[0].sha256_hex, piece_idx, offset, length);
        }
    }
}
...
static int command_seed(const char *dir, uint16_t port, const char *gate_key,
    ...
    if (init_peer_db(&state, index_path) < 0) {
