...
struct tokenizer {
    ...
    /* ByteLevel tokenizer support (GPT-2/Qwen family) */
    int      byte_level;
    char     byte_piece[256][8];
    uint8_t  byte_piece_len[256];
    uint32_t byte_codepoint[256];
};
...
static int utf8_encode_codepoint(uint32_t cp, char out[8]) {
    if (cp < 0x80) {
        out[0] = (char)cp;
        return 1;
    }
...
static int utf8_decode_codepoint(const char *s, size_t len, size_t *consumed, uint32_t *cp_out) {
    unsigned char c0;
    if (!s || len == 0 || !consumed || !cp_out) return -1;
...
static void init_byte_level_maps(tokenizer_t *tok) {
    int used[256] = {0};
    int order[256];
    uint32_t cps[256];
    int n = 0;
    ...
    for (int i = 0; i < n; i++) {
        int b = order[i];
        tok->byte_codepoint[b] = cps[i];
        tok->byte_piece_len[b] = (uint8_t)utf8_encode_codepoint(cps[i], tok->byte_piece[b]);
        tok->byte_piece[b][tok->byte_piece_len[b]] = '\0';
    }
}

static int byte_level_codepoint_to_byte(const tokenizer_t *tok, uint32_t cp) {
    if (!tok) return -1;
    for (int b = 0; b < 256; b++) {
        if (tok->byte_codepoint[b] == cp) return b;
    }
    return -1;
}
...
static void tok_resolve_special_ids(tokenizer_t *tok) {
    int id;
    if (!tok) return;

    id = str_map_get(tok->str_map, tok->str_map_size, "<unk>");
    if (id >= 0) tok->unk_id = id;

    id = str_map_get(tok->str_map, tok->str_map_size, "<s>");
    if (id >= 0) tok->bos_id = id;

    id = str_map_get(tok->str_map, tok->str_map_size, "</s>");
    if (id >= 0) tok->eos_id = id;

    if (tok->byte_level) {
        id = str_map_get(tok->str_map, tok->str_map_size, "<|endoftext|>");
        if (id >= 0 && tok->eos_id < 0) tok->eos_id = id;
    }
}
...
tokenizer_t *tok_load(const char *json_path) {
    ...
    tok->byte_level = strstr(model_start, "\"ByteLevel\"") != NULL ||
                      strstr(json, "\"ByteLevel\"") != NULL;
    if (tok->byte_level) init_byte_level_maps(tok);
    ...
    /* Some tokenizers (e.g. Qwen) place high-ID tokens in added_tokens.
     * Expand id-space so tok_id_to_str can decode those generated IDs. */
    int max_added_id = -1;
    {
        ...
        if (at_start && *at_start == '[') {
            ...
            while (*p && *p != ']') {
                ...
                while (*p && *p != '}') {
                    ...
                    if (*p != '"') break;
                    p++;
                    p = parse_json_string(p, key_buf, sizeof(key_buf));
                    while (*p && *p != ':' && *p != '}') p++;
                    if (*p != ':') break;
                    p++;
                    while (*p == ' ' || *p == '\t') p++;
                    if (strcmp(key_buf, "id") == 0) {
                        int at_id = (int)strtol(p, NULL, 10);
                        if (at_id > max_added_id) max_added_id = at_id;
                    }
                    while (*p && *p != ',' && *p != '}') p++;
                    if (*p == ',') p++;
                }
    ...
    int full_vocab_size = vocab_size;
    if (max_added_id + 1 > full_vocab_size) full_vocab_size = max_added_id + 1;

    tok->vocab_size = full_vocab_size;
    tok->id_to_str  = (char **)calloc((size_t)full_vocab_size, sizeof(char *));
    tok->id_is_byte = (uint8_t *)calloc((size_t)full_vocab_size, 1);
    tok->id_byte_val= (uint8_t *)calloc((size_t)full_vocab_size, 1);

    /* Allocate string pool: rough estimate 32 bytes avg */
    tok->str_pool_size = (size_t)full_vocab_size * 32 + 65536;
    ...
    tok->str_map = (tok_hash_entry_t **)calloc((size_t)tok->str_map_size,
        ...
        tok_hash_entry_t *entry_pool = (tok_hash_entry_t *)malloc(
            (size_t)full_vocab_size * sizeof(tok_hash_entry_t));
    ...
    if (merges_start && *merges_start == '[' && n_merges > 0) {
        ...
        while (*p && *p != ']' && rank < n_merges) {
            p = skip_ws(p);
            char merge_left[512];
            char merge_right[512];
            const char *s1 = NULL;
            const char *s2 = NULL;
            if (*p == '"') {
                char merge_str[512];
                p++;
                p = parse_json_string(p, merge_str, sizeof(merge_str));
                while (*p && *p != ',' && *p != ']') p++;
                if (*p == ',') p++;
                ...
                *space = '\0';
                ...
                if (*p != '"') continue;
                p++;
                p = parse_json_string(p, merge_left, sizeof(merge_left));
                p = skip_ws(p);
                if (*p == ',') p++;
                p = skip_ws(p);
                if (*p != '"') continue;
                p++;
                p = parse_json_string(p, merge_right, sizeof(merge_right));
                while (*p && *p != ']') p++;
                if (*p == ']') p++;
                while (*p && *p != ',' && *p != ']') p++;
                if (*p == ',') p++;
    ...
    /* ── Parse added_tokens: collect special tokens for pre-scan ── */
    {
        ...
        if (n_at > 0) {
            ...
            while (*p && *p != ']' && tok->n_special < n_at) {
                ...
                if (*p == '}') p++;
                if (at_id >= 0 && at_id < tok->vocab_size && at_content[0] && !tok->id_to_str[at_id]) {
                    size_t clen = strlen(at_content) + 1;
                    if (pool_offset + clen > tok->str_pool_size) {
                        while (pool_offset + clen > tok->str_pool_size) tok->str_pool_size *= 2;
                        tok->str_pool = (char *)realloc(tok->str_pool, tok->str_pool_size);
                    }
                    memcpy(tok->str_pool + pool_offset, at_content, clen);
                    tok->id_to_str[at_id] = tok->str_pool + pool_offset;
                    str_map_insert(tok->str_map, tok->str_map_size,
                                   tok->str_pool + pool_offset,
                                   at_id, entry_pool, at_id);
                    pool_offset += clen;
                }
    ...
    free(json);
    tok_resolve_special_ids(tok);

    fprintf(stderr, "tok_load: vocab=%d (base=%d) merges=%d bos=%d eos=%d special=%d byte_level=%d\n",
            tok->vocab_size, vocab_size, n_merges, tok->bos_id, tok->eos_id, tok->n_special,
            tok->byte_level);
    return tok;
}
...
static int *tok_encode_byte_level(tokenizer_t *tok, const char *text, int add_bos, int *n_out) {
    size_t text_len;
    int *tokens;
    int ntok = 0;
    size_t i = 0;
    ...
    tokens = (int *)malloc((text_len + 8) * sizeof(int));
    ...
    while (i < text_len) {
        ...
        for (int si = 0; si < tok->n_special; si++) {
            ...
            if (splen > 0 && i + splen <= text_len &&
                memcmp(text + i, sp, splen) == 0) {
        ...
        int id = str_map_get(tok->str_map, tok->str_map_size, tok->byte_piece[b]);
        tokens[ntok++] = (id >= 0) ? id : tok->unk_id;
    }
...
int *tok_encode(tokenizer_t *tok, const char *text, int add_bos, int *n_out) {
    if (!tok || !text || !n_out) return NULL;
    if (tok->byte_level) return tok_encode_byte_level(tok, text, add_bos, n_out);
...
char *tok_decode(tokenizer_t *tok, const int *ids, int n) {
    ...
    if (tok->byte_level) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (jpos + slen + 1 > cap) {
                while (jpos + slen + 1 > cap) cap *= 2;
                joined = (char *)realloc(joined, cap);
                out = (char *)realloc(out, cap);
            }
            if (slen > 0) {
                memcpy(joined + jpos, s, slen);
                jpos += slen;
            }
        }
        joined[jpos] = '\0';
        for (size_t i = 0; i < jpos;) {
            uint32_t cp = 0;
            size_t consumed = 0;
            if (utf8_decode_codepoint(joined + i, jpos - i, &consumed, &cp) != 0 || consumed == 0) {
                consumed = 1;
                cp = (unsigned char)joined[i];
            }
            i += consumed;
            int b = byte_level_codepoint_to_byte(tok, cp);
            if (b >= 0) {
                if (opos + 2 > cap) {
                    cap *= 2;
                    out = (char *)realloc(out, cap);
                ...
                char enc[8];
                int elen = utf8_encode_codepoint(cp, enc);
                if (opos + (size_t)elen + 1 > cap) {
                    while (opos + (size_t)elen + 1 > cap) cap *= 2;
                    out = (char *)realloc(out, cap);
                }
...
int tok_find_id(tokenizer_t *tok, const char *text) {
    if (!tok || !text) return -1;
    return str_map_get(tok->str_map, tok->str_map_size, text);
}
