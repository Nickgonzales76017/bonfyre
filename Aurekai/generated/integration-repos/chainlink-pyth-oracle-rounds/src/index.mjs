...
export function mapHostEventToKernel(event) {
  const committedState = createStateSnapshot({
    chart_family: "oracle_round",
    policy_family: "oracle",
    payload: { target: "Chainlink/Pyth oracle rounds", event },
  });
  ...
  const transitions = createTransitionSequence(["prepare", "commit"], {
    ...
    metadata: { target: "Chainlink/Pyth oracle rounds" },
  });

  const trajectory = createTrajectory({
    target: "Chainlink/Pyth oracle rounds",
    transitions,
    lineage_edges: [],
  });
