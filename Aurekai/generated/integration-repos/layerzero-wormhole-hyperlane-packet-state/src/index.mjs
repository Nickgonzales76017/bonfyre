...
export function mapHostEventToKernel(event) {
  const committedState = createStateSnapshot({
    ...
    payload: { target: "LayerZero/Wormhole/Hyperlane packet state", event },
  });
  ...
  const transitions = createTransitionSequence(["prepare", "commit"], {
    ...
    metadata: { target: "LayerZero/Wormhole/Hyperlane packet state" },
  });

  const trajectory = createTrajectory({
    target: "LayerZero/Wormhole/Hyperlane packet state",
    transitions,
    lineage_edges: [],
  });
