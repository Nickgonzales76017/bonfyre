...
function inferPolicyFamily(category, target, surface) {
  ...
  if (key.includes("commerce") || targetKey.includes("maker") || targetKey.includes("ondo")) return "treasury";
  if (targetKey.includes("hook")) return "hook_safety";
...
function readStateMachineMappings(repoRoot) {
  const path = join(repoRoot, "registry", "state-machine-mappings.json");
  if (!existsSync(path)) return { groups: {}, targets: {} };
...
function resolveStateMachineMapping(mappings, stateMachineType, target) {
  const targetKey = normalizeKey(target);
  const byTarget = mappings.targets?.[targetKey] || null;
  const byGroup = mappings.groups?.[stateMachineType] || null;
  return {
    chart_family: byTarget?.chart_family || byGroup?.chart_family || null,
    policy_family: byTarget?.policy_family || byGroup?.policy_family || null,
    lineage_edge_family: byTarget?.lineage_edge_family || byGroup?.lineage_edge_family || null,
    claim_families: Array.isArray(byTarget?.claim_families)
      ? byTarget.claim_families
      : Array.isArray(byGroup?.claim_families)
        ? byGroup.claim_families
        : [],
    commerce_profile: byTarget?.commerce_profile || byGroup?.commerce_profile || "none",
  };
}
...
function buildTargetIndex(repoRoot, registry) {
  const mappings = readStateMachineMappings(repoRoot);
  const index = new Map();
  ...
  for (const [category, targets] of Object.entries(hardCategories)) {
    for (const target of targets) {
      const descriptor = {
        ...
        policyFamily: inferPolicyFamily(category, target, "hard-fork"),
        lineageEdgeFamily: inferEdgeFamily(category, target, "hard-fork"),
        claimFamilies: ["continuity_bound"],
        commerceProfile: "none",
      };
      const memberships = buildStateMachineMembership(registry, target);
      descriptor.stateMachineTypes = memberships;
      const mapping = resolveStateMachineMapping(mappings, memberships[0] || "", target);
      if (mapping.chart_family) descriptor.chartFamily = mapping.chart_family;
      if (mapping.policy_family) descriptor.policyFamily = mapping.policy_family;
      if (mapping.lineage_edge_family) descriptor.lineageEdgeFamily = mapping.lineage_edge_family;
      if (mapping.claim_families.length > 0) descriptor.claimFamilies = mapping.claim_families;
      if (mapping.commerce_profile) descriptor.commerceProfile = mapping.commerce_profile;
  ...
  for (const target of registry.soft_fork_targets?.targets || []) {
    const descriptor = {
      ...
      policyFamily: inferPolicyFamily("runtime-shim", target, "soft-fork"),
      lineageEdgeFamily: inferEdgeFamily("runtime-shim", target, "soft-fork"),
      claimFamilies: ["continuity_bound", "workflow_policy_compliant"],
      commerceProfile: "none",
    };
    const memberships = buildStateMachineMembership(registry, target);
    descriptor.stateMachineTypes = memberships;
  ...
  for (const target of registry.ingress_adapters?.thin_kernel_projectors || []) {
    const descriptor = {
      ...
      policyFamily: inferPolicyFamily("thin-kernel-projector", target, "adapter"),
      lineageEdgeFamily: inferEdgeFamily("thin-kernel-projector", target, "adapter"),
      claimFamilies: ["continuity_bound"],
      commerceProfile: "none",
    };
    const memberships = buildStateMachineMembership(registry, target);
  ...
  for (const target of registry.ingress_adapters?.protocol_observer_layers || []) {
    const descriptor = {
      ...
      policyFamily: inferPolicyFamily("protocol-observer-layer", target, "observer"),
      lineageEdgeFamily: inferEdgeFamily("protocol-observer-layer", target, "observer"),
      claimFamilies: ["continuity_bound", "attestation_consistency_bound"],
      commerceProfile: "none",
    };
    const memberships = buildStateMachineMembership(registry, target);
    descriptor.stateMachineTypes = memberships;
  ...
  for (const [group, targets] of Object.entries(stateMachine)) {
    for (const target of targets) {
      const mapping = resolveStateMachineMapping(mappings, group, target);
      const descriptor = {
        target,
        ...
        chartFamily: mapping.chart_family || inferChartFamily(group, target, "state-machine"),
        policyFamily: mapping.policy_family || inferPolicyFamily(group, target, "state-machine"),
        lineageEdgeFamily: mapping.lineage_edge_family || inferEdgeFamily(group, target, "state-machine"),
        claimFamilies: mapping.claim_families,
        commerceProfile: mapping.commerce_profile,
      };
...
function buildProtocolProfile(descriptor) {
  const policyFamily = descriptor.policyFamily || inferPolicyFamily(descriptor.category, descriptor.target, descriptor.surface);
  const base = {
    maps_into: [
      "committed_state",
      "transitions",
...
function makeSpecializedClaims(descriptor, event, committedState, policyDecision, trajectory, runContext) {
  ...
  if (family === "settlement") {
    claims.push({
      ...
      claim_family: family,
      state_machine_type: descriptor.stateMachineTypes?.[0] || null,
      ...
      evaluator: "settlement-ack-evaluator",
      inputs: { ack: event?.ack, acknowledged: event?.acknowledged },
  ...
  if (family === "oracle") {
    ...
    claims.push({
      ...
      evaluator: "oracle-drift-evaluator",
      inputs: { observed_drift: observed, max_drift: maxDrift },
  ...
  if (family === "collateral") {
    ...
    claims.push({
      ...
      evaluator: "collateralization-evaluator",
      inputs: { collateral_ratio: ratio, min_collateral_ratio: min },
  ...
  if (family === "workflow") {
    claims.push({
      ...
      evaluator: "workflow-policy-evaluator",
      inputs: { decision: policyDecision.decision },
...
function makeLineageEdges(descriptor, event, committedState, transitions, mutationBoundary, runContext) {
  const profile = buildProtocolProfile(descriptor);
  const edgeFamily = descriptor.lineageEdgeFamily || mutationBoundary?.lineage_edge_type || inferEdgeFamily(descriptor.category, descriptor.target, descriptor.surface);
  ...
  const base = {
    ...
    edge_kind: descriptor.surface === "observer" ? "attestation" : "causal",
    edge_role: "runtime-lineage",
    state_machine_type: descriptor.stateMachineTypes?.[0] || null,
  ...
  return [
    {
      ...
      edge_kind: "derivation",
      edge_role: "ingress-projection",
      from: sourceNode,
      to: targetNode,
    ...
    },
    {
      ...
      edge_kind: "causal",
      edge_role: "state-transition-link",
      from: targetNode,
...
function makeExportProjections(descriptor, event, committedState, transitions, lineageEdges, policyDecision, transportSemantics, commerceContinuity, runContext) {
  const profile = buildProtocolProfile(descriptor);
  ...
  if (commerceContinuity?.treasury_continuity) {
    projections.push({
      schema_version: "aurekai.export_projection.v1",
      projection_id: `${slugifyTarget(descriptor.target)}:${runContext.runId}:treasury-continuity`,
      format: "application/json",
      intent: "treasury-continuity-projection",
      payload: {
        treasury_continuity: commerceContinuity.treasury_continuity,
        invoice_proof: commerceContinuity.invoice_proof,
        chargeability_claim: commerceContinuity.chargeability_claim,
      },
    });
  }
...
function makeTrajectory(descriptor, transitions, lineageEdges, mutationBoundary, runContext) {
  ...
  const calculus = buildTrajectoryCalculus({
    ...
    defaultQueries: [
      ...
      { type: "branch", edge_family: descriptor.lineageEdgeFamily || inferEdgeFamily(descriptor.category, descriptor.target, descriptor.surface) },
    ],
  });
  ...
  return {
    ...
    continuity_summary: {
      ingress_depth: descriptor.ingressDepth,
      edge_family: descriptor.lineageEdgeFamily || mutationBoundary?.lineage_edge_type || inferEdgeFamily(descriptor.category, descriptor.target, descriptor.surface),
      policy_family: descriptor.policyFamily || inferPolicyFamily(descriptor.category, descriptor.target, descriptor.surface),
...
function makeCommerceContinuityBundle(descriptor, event, residualAssessment, committedState, transitions, policyDecision, runContext) {
  if (!isCommerceDescriptor(descriptor, event) && descriptor.commerceProfile !== "treasury") return null;
  ...
  const treasuryContinuity = {
    schema_version: "aurekai.treasury_continuity.v1",
    treasury_id: `${slugifyTarget(descriptor.target)}:treasury`,
    continuity_state_id: committedState.state_id,
    invoice_id: invoiceId,
    chargeability_claim_id: chargeabilityClaim.claim_id,
    policy_family: policyDecision.policy_family || descriptor.policyFamily || "treasury",
    continuity_verdict: chargeabilityEvaluation.evaluation_result,
    continuity_budget: Number(event?.continuity_budget ?? 1),
    emitted_at: runContext.timestamp,
    references: {
      fulfillment_transition_id: fulfillmentTransition.transition_id,
      delivery_witness_id: deliveryWitness.witness_id,
      proof_bundle_ref: proofBundleRef,
    },
  };
  ...
  return {
    ...
    treasury_continuity: treasuryContinuity,
  };
}
...
function makeFunctionalClaims(descriptor, event, residualAssessment, committedState, policyDecision, trajectory, commerceContinuity, runContext) {
  ...
  const continuityFamily = descriptor.claimFamilies?.[0] || descriptor.policyFamily || inferPolicyFamily(descriptor.category, descriptor.target, descriptor.surface);
  ...
  const claims = [
    {
      ...
      evaluator: "policy-family-evaluator",
      inputs: {
        threshold,
        residual_score: residualAssessment?.composite_severity,
        decision: policyDecision.decision,
      },
      context: {
        policy_family: descriptor.policyFamily || inferPolicyFamily(descriptor.category, descriptor.target, descriptor.surface),
  ...
  if (commerceContinuity) {
    claims.push({
      ...
      claim_family: descriptor.claimFamilies?.find((family) => family === "treasury") || "treasury",
      state_machine_type: descriptor.stateMachineTypes?.[0] || null,
      ...
      evaluator: "commerce-continuity-evaluator",
      inputs: {
        invoice_id: commerceContinuity.invoice_proof.invoice_id,
        chargeability_result: commerceContinuity.chargeability_claim.evaluation_result,
      },
      context: {
        invoice_id: commerceContinuity.invoice_proof.invoice_id,
...
export function listIntegrationTargets(repoRoot, options = {}) {
  ...
  const index = buildTargetIndex(repoRoot, registry);
  const rows = [];
  ...
  for (const descriptor of index.values()) {
    rows.push({
      ...
      required_kernel_layers: descriptor.requiredKernelLayers,
      chart_family: descriptor.chartFamily,
      policy_family: descriptor.policyFamily,
      lineage_edge_family: descriptor.lineageEdgeFamily,
      claim_families: descriptor.claimFamilies,
      commerce_profile: descriptor.commerceProfile,
      state_machine_types: descriptor.stateMachineTypes,
    });
  }
