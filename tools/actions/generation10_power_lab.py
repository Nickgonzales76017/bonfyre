...
barrier.  The QUIC/OpenSSL backend is exercised by the separate physical
fabric lane, so a transport dependency cannot suppress evidence from the
remaining public Power estate.
"""
...
def command_dirs() -> list[str]:
    ...
    if not root.is_dir():
        return []
    return sorted(p.name for p in root.iterdir() if p.is_dir() and p.name.startswith("Bonfyre") and (p / "Makefile").is_file())
...
def manifest_observation(commands: list[str]) -> dict[str, Any]:
    ...
    if manifest_path.is_file():
        try:
            ...
            manifest_dirs = sorted({str(item.get("command_dir")) for item in value.get("commands", []) if item.get("command_dir")})
    ...
    return {
        ...
        "manifest_command_count": declared_count,
        "live_not_manifest": sorted(live - manifest),
        "manifest_not_live": sorted(manifest - live),
    }


def run_make(directory: Path, timeout: int, cflags: str) -> dict[str, Any]:
...
def build_foundation(timeout: int) -> list[dict[str, Any]]:
    ...
    return [
        run_make(ROOT / "lib" / "liblambda-tensors", timeout, flags),
        run_make(ROOT / "lib" / "libbonfyre", timeout, flags),
    ]


def build_power(name: str, timeout: int) -> dict[str, Any]:
    directory = ROOT / "cmd" / name
    result = run_make(directory, timeout, "-O2 -Wall -Wextra -std=c11 -D_DEFAULT_SOURCE")
    result["command"] = name
...
def receipt_base(subject: str) -> dict[str, Any]:
    ...
    return {
        ...
        "kind": "power_shard",
        "subject": subject,
        "git_sha": os.environ.get("BONFYRE_SOURCE_SHA") or os.environ.get("GITHUB_SHA") or git("rev-parse", "HEAD").stdout.strip(),
