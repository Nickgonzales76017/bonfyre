...
def run_test_file(path: str, timeout: int) -> dict[str, Any]:
    full = ROOT / path; started = time.monotonic(); argv = ["bash", str(full)] if full.suffix == ".sh" else [sys.executable, str(full)]
    env = os.environ.copy(); env.update({"BONFYRE_CI":"1", "BONFYRE_CI_NO_EXTERNAL_EFFECTS":"1", "BONFYRE_AUTHORITY":"observe", "NO_COLOR":"1"})
    try:
...
def cmd_run_circuit(args: argparse.Namespace) -> int:
    ...
    if args.max_tests > 0: tests = tests[:args.max_tests]
    results = [run_test_file(test,args.timeout) for test in tests]; failures=[r for r in results if r["outcome"]!="passed"]
    receipt=receipt_base("semantic_circuit",args.circuit); receipt.update({"owners":contract["circuits"][args.circuit]["owners"],"tests_selected":tests,"results":results,"coverage_debt":[] if tests else ["no executable requirement test matched this circuit"],"outcome":"failed" if failures else ("coverage_debt" if not tests else "passed")})
    print(write_receipt(receipt,Path(args.output_dir))); return 1 if failures else 0


def build_native_foundations(timeout: int) -> dict[str, Any]:
    started=time.monotonic(); env=os.environ.copy(); env.update({"CFLAGS":"-std=c11 -O2 -Wall -Wextra","BONFYRE_CI":"1","BONFYRE_CI_NO_EXTERNAL_EFFECTS":"1"}); targets=["Core","Agents","Data","Logic","Media","Voice"]
    try:
        proc=subprocess.run(["make","-j2",*targets],cwd=ROOT,text=True,capture_output=True,env=env,timeout=timeout)
...
def build_one_power(command: str, timeout: int) -> dict[str, Any]:
    directory=ROOT/"cmd"/command; started=time.monotonic(); env=os.environ.copy(); env.update({"CFLAGS":"-std=c11 -O2 -Wall -Wextra","BONFYRE_CI":"1","BONFYRE_CI_NO_EXTERNAL_EFFECTS":"1"})
    try:
...
def cmd_run_power_shard(args: argparse.Namespace) -> int:
    ...
    if args.scope=="changed":
        ...
        for path in changed_paths(args.base,args.head):
            ...
            if len(parts)>=2 and parts[0]=="cmd" and parts[1].startswith("Bonfyre"): changed.add(parts[1])
        commands=sorted(changed & set(commands))
    assigned=[cmd for i,cmd in enumerate(commands) if i%args.total==args.shard]
    foundation=build_native_foundations(args.foundation_timeout); results=[] if foundation["outcome"]!="passed" else [build_one_power(cmd,args.timeout) for cmd in assigned]
    failures=[r for r in results if r["outcome"]!="passed"] + ([] if foundation["outcome"]=="passed" else [foundation])
    receipt=receipt_base("power_shard",f"powers-{args.shard}-of-{args.total}"); receipt.update({"canonical_public_power_count":contract["canonical_counts"]["public_powers"],"inventory_observation":inventory_contract_observation(contract),"scope":args.scope,"commands_assigned":assigned,"native_foundation":foundation,"results":results,"outcome":"failed" if failures else "passed"})
    print(write_receipt(receipt,Path(args.output_dir))); return 1 if failures else 0


def reject_effect(effect: dict[str, Any]) -> list[str]:
    errors=[]
    if effect.get("exit_code")==0 and not effect.get("terminal_process_occurrence"): errors.append("exit_zero_without_terminal_process_occurrence")
    if effect.get("external_effect") and not effect.get("authority_granted"): errors.append("external_effect_without_authority")
    if effect.get("external_effect") and not effect.get("receipt"): errors.append("external_effect_without_receipt")
    if effect.get("factor_sha")!=effect.get("current_factor_sha"): errors.append("stale_factor_envelope")
    if effect.get("child") and effect.get("child_parent")!=effect.get("parent"): errors.append("broken_parent_child")
    if effect.get("model") and effect.get("provider_model")!=effect.get("model"): errors.append("model_provider_mismatch")
    if effect.get("effect_occurrences",1)>1 and not effect.get("dedupe_receipt"): errors.append("duplicate_effect_occurrence")
    if effect.get("semantic_gc") and not effect.get("deletion_proof"): errors.append("semantic_gc_without_deletion_proof")
    return errors
...
def fault_fixture(name: str) -> tuple[dict[str, Any], str]:
    ...
    if name=="cli_exit_zero_without_witness": base["terminal_process_occurrence"]=False; expected="exit_zero_without_terminal_process_occurrence"
    elif name=="effect_without_receipt": base["external_effect"]=True; base["authority_granted"]=True; expected="external_effect_without_receipt"
    elif name=="stale_factor_envelope": base["current_factor_sha"]="factor-b"
    elif name=="authority_leak": base["external_effect"]=True; base["receipt"]="receipt-1"; expected="external_effect_without_authority"
    elif name=="broken_parent_child": base["child"]="work-2"; base["child_parent"]="wrong-parent"
    elif name=="model_provider_mismatch": base["model"]="model-a"; base["provider_model"]="model-b"
    elif name=="duplicate_effect_occurrence": base["effect_occurrences"]=2
    elif name=="semantic_gc_without_deletion_proof": base["semantic_gc"]=True
...
def cmd_fault(args: argparse.Namespace) -> int:
    contract=load_contract()
    if args.scenario not in contract["fault_scenarios"]: return 2
    fixture,expected=fault_fixture(args.scenario); rejected=reject_effect(fixture); passed=expected in rejected
    receipt=receipt_base("semantic_fault",args.scenario); receipt.update({"expected_rejection":expected,"observed_rejections":rejected,"fail_closed":passed,"outcome":"passed" if passed else "failed"}); print(write_receipt(receipt,Path(args.output_dir))); return 0 if passed else 1
...
def cmd_bundle(args: argparse.Namespace) -> int:
    ...
    for path in iter_receipts(source):
        ...
        except (OSError,json.JSONDecodeError): payload={"outcome":"malformed"}
    ...
    bundle["bundle_sha256"]=sha256_value(bundle); out=Path(args.output); out.parent.mkdir(parents=True,exist_ok=True); out.write_bytes(canonical_bytes(bundle))
    summary=Path(args.summary); summary.parent.mkdir(parents=True,exist_ok=True); summary.write_text("\n".join(["# Generation-10 ProofBundle","",f"- generation: `{bundle['generation']}` / `{bundle['version']}`",f"- receipts: **{bundle['receipt_count']}**",f"- outcomes: `{json.dumps(outcomes,sort_keys=True)}`",f"- proof complete: **{str(bundle['proof_complete']).lower()}**",f"- bundle sha256: `{bundle['bundle_sha256']}`",f"- git shas: `{', '.join(git_shas) or 'none'}`","","A green bundle is valid only for the exact source SHA and Generation-10 contract hash above.",""]),encoding="utf-8")
    print(json.dumps(bundle,indent=2)); return 0 if bundle["proof_complete"] else 1


def cmd_contract(_: argparse.Namespace) -> int:
    contract=load_contract(); errors=validate_contract(contract); print(json.dumps({"contract_valid":not errors,"errors":errors,"contract_sha256":sha256_file(CONTRACT_PATH),"inventory_observation":inventory_contract_observation(contract)},indent=2)); return 0 if not errors else 1
